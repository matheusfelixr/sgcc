...
public class SecurityController {
    ...
    @ApiOperation(value = "Método responsável por editar senha. ")
    public ResponseEntity<ResponseApi<MessageDTO>> editPassword(@RequestBody EditPasswordRequestDTO editPasswordRequestDTO) throws Exception {
        LOGGER.info("Inicio processo de criação de nova senha.");
        ResponseApi<MessageDTO> response = new ResponseApi<>();
        try {
            UserAuthentication currentUser = securityService.getCurrentUser();

            response.setData(this.securityService.editPassword(editPasswordRequestDTO, currentUser));
            LOGGER.info("v de  senha realizada com sucesso.");
            return ResponseEntity.ok(response);
        } catch (ValidationException e) {
            LOGGER.error(e.getMessage());
            response.setErrors(Arrays.asList(e.getMessage()));
            ...
            e.printStackTrace();
            LOGGER.error("Erro inesperado ao tentar editar senha");
            List<String> errors = Arrays.asList("Erro inesperado ao tentar editar senha");
            response.setErrors(errors);
            return ResponseEntity.ok(response);
        }
    }
